...
Constrained ordination is the marriage of basic ordination methods and
regression

We try to extract pattern from our responses but we extract only that
which can be explained by the measured covariates

We can view RDA as being the result of

\begin{itemize}
\tightlist
\item
  fitting a multiple regression to each species in turn using the set of
  covariates,
\item
  then running PCA on the matrix of fitted values from those regressions
\end{itemize}

These are regression models so work with them as if they were a linear
model

But we have little theory, so all inference is done using
\alert{permutation} tests
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{vif.cca}\NormalTok{(vare.cca)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
        Al          P          K   Baresoil        P:K P:Baresoil 
  1.335298  17.989518  12.891913  12.927791  41.037782  13.426405 
\end{verbatim}
...
\emph{Small} individuals on basis of carapace length)
...
\begin{Highlighting}[]
\NormalTok{## load data}
\NormalTok{crayfish <-}\StringTok{ }\KeywordTok{head}\NormalTok{(}\KeywordTok{read.csv}\NormalTok{(}\StringTok{"crayfish-spp.csv"}\NormalTok{)[, -}\DecValTok{1}\NormalTok{], -}\DecValTok{1}\NormalTok{)}
\NormalTok{design <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"crayfish-design.csv"}\NormalTok{, }\DataTypeTok{skip =} \DecValTok{1}\NormalTok{)[, -}\DecValTok{1}\NormalTok{]}
...
CA would just delete these rows as they contain no data

Fit unconstrained PCA
...
\KeywordTok{layout}\NormalTok{(}\KeywordTok{matrix}\NormalTok{(}\DecValTok{1}\NormalTok{:}\DecValTok{3}\NormalTok{, }\DataTypeTok{ncol =} \DecValTok{3}\NormalTok{))}
\KeywordTok{biplot}\NormalTok{(m.pca, }\DataTypeTok{type =} \KeywordTok{c}\NormalTok{(}\StringTok{"text"}\NormalTok{, }\StringTok{"none"}\NormalTok{), }\DataTypeTok{display =} \StringTok{"species"}\NormalTok{, }\DataTypeTok{scaling =} \StringTok{"species"}\NormalTok{, }\DataTypeTok{correlation =} \OtherTok{TRUE}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(}\KeywordTok{envfit}\NormalTok{(m.pca ~}\StringTok{ }\NormalTok{Watershed, }\DataTypeTok{data =} \NormalTok{design, }\DataTypeTok{scaling =} \StringTok{"species"}\NormalTok{, }\DataTypeTok{correlation =} \OtherTok{TRUE}\NormalTok{), }\DataTypeTok{add =} \OtherTok{FALSE}\NormalTok{,}
     \DataTypeTok{labels =} \KeywordTok{levels}\NormalTok{(design$Watershed))}
\NormalTok{ev.pca <-}\StringTok{ }\KeywordTok{envfit}\NormalTok{(m.pca ~}\StringTok{ }\NormalTok{Stream, }\DataTypeTok{data =} \NormalTok{design, }\DataTypeTok{scaling =} \StringTok{"species"}\NormalTok{, }\DataTypeTok{correlation =} \OtherTok{TRUE}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(ev.pca, }\DataTypeTok{labels =} \KeywordTok{levels}\NormalTok{(design$Stream), }\DataTypeTok{add =} \OtherTok{FALSE}\NormalTok{)}
\KeywordTok{layout}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=\linewidth]{02-constrained-ordination_files/figure-beamer/crayfish-pca-plot-1} \end{center}
...
\begin{frame}{Crayfish Summary}

\begin{longtable}[]{@{}lrrrr@{}}
\toprule
Component & Explained \% & DF & `Mean Square' & p value\tabularnewline
\midrule
\endhead
Watershed & 18.9 & 6 & 3.150 & 0.002\tabularnewline
Stream & 12.3 & 14 & 0.879 & 0.008\tabularnewline
Reach & 15.8 & 42 & 0.376 & 0.002\tabularnewline
Run & 19.5 & 126 & 0.155 & 0.002\tabularnewline
Residual & 33.5 & 378 & 0.089 & NA\tabularnewline
Total & 100.0 & 566 & 0.177 & NA\tabularnewline
\bottomrule
\end{longtable}
