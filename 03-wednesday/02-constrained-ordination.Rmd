...
output:
  beamer_presentation:
    ...
    highlight: tango
    slide_level: 2
    template: ~/work/teaching/slides-template-v2.tex
    fig_width: 6
    fig_height: 3.5
    fig_crop: false
    pandoc_args: [
...
Constrained ordination is the marriage of basic ordination methods and regression

We try to extract pattern from our responses but we extract only that which can be explained by the measured covariates

We can view RDA as being the result of

1. fitting a multiple regression to each species in turn using the set of covariates,
2. then running PCA on the matrix of fitted values from those regressions

These are regression models so work with them as if they were a linear model

But we have little theory, so all inference is done using \alert{permutation} tests
...
## load the data
spp <- read.csv("../00-data-sets/meadow-spp.csv", header = TRUE, row.names = 1)
env <- read.csv("../00-data-sets/meadow-env.csv", header = TRUE, row.names = 1)
```
\normalsize
...
## load the data
spp <- read.csv("../00-data-sets/ohraz-spp.csv", header = TRUE, row.names = 1)
env <- read.csv("../00-data-sets/ohraz-env.csv", header = TRUE, row.names = 1)
molinia <- spp[, 1]
spp <- spp[, -1]
...
A firt step in understanding this variation is to test for its exisistence
...
567 records of 5 species (each sub-divided into *Large* & *Small* individuals on basis of carapace length)

## Hierarchical analysis of crayfish
```{r load-crayfish-real, echo=FALSE, results="hide"}
## load data
crayfish <- head(read.csv("../00-data-sets/crayfish-spp.csv")[, -1], -1)
design <- read.csv("../00-data-sets/crayfish-design.csv", skip = 1)[, -1]
...
```
```{r load-crayfish, eval=FALSE}
## load data
crayfish <- head(read.csv("crayfish-spp.csv")[, -1], -1)
design <- read.csv("crayfish-design.csv", skip = 1)[, -1]
...
A number of samples have 0 crayfish, which excludes unimodal methods --- CA would just delete these rows as they contain no data
...
## Hierarchical analysis of crayfish --- Unconstrained
\tiny
```{r crayfish-pca-plot, fig.width = 16, fig.height = 5, fig.show = "hold", crop.plot = TRUE, out.width="\\linewidth"}
set.seed(23)
layout(matrix(1:3, ncol = 3))
biplot(m.pca, type = c("text", "none"), display = "species", scaling = "species", correlation = TRUE)
plot(envfit(m.pca ~ Watershed, data = design, scaling = "species", correlation = TRUE), add = FALSE,
     labels = levels(design$Watershed))
ev.pca <- envfit(m.pca ~ Stream, data = design, scaling = "species", correlation = TRUE)
plot(ev.pca, labels = levels(design$Stream), add = FALSE)
layout(1)
```
\normalsize
...
```{r crayfish-summary-tab, results="asis", echo=FALSE}
df <- data.frame(Component = c("Watershed","Stream","Reach","Run","Residual","Total"),
                 "Explained %" = c(18.9,12.3,15.8,19.5,33.5,100),
                 "DF" = c(6,14,42,126,378,566), check.names = FALSE)
df <- transform(df, "'Mean Square'" = round(`Explained %` / DF, 3),
                "p value" = c(0.002, 0.008, 0.002, 0.002, NA, NA),
                check.names = FALSE)
knitr::kable(df)
```
...
Copyright \textcopyright (2015--16) Gavin L. Simpson *Some Rights Reserved*
